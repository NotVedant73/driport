...
import { Star, Heart, Truck, RefreshCw, Shield, Share2, ChevronLeft } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  const product = useMemo(() => {
    ...
    const image = backendProduct?.image || "https://images.unsplash.com/photo-1666861585341-5bd1e7b1ed71?auto=format&fit=crop&w=1080&q=80";
    ...
    <div className="bg-amber-50/30 min-h-screen">
      <div className="container mx-auto px-4 py-8">
              ...
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`bg-white rounded-lg overflow-hidden border-2 ${
                    mainImage === image ? 'border-amber-900' : 'border-amber-900/10'
          ...
          <div>
            <div className="bg-white rounded-lg p-8 border-2 border-amber-900/10">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="text-sm text-amber-700 mb-2">{product.category}</div>
                  <h1 className="text-4xl font-serif text-amber-900 mb-4">{product.name}</h1>
                </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-amber-800">({product.reviews} reviews)</span>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <a href="#" className="text-sm text-amber-700 hover:text-amber-900">Size Guide</a>
                </div>
              ...
              {/* Quantity */}
              <div className="mb-8">
                <label className="font-semibold text-amber-900 block mb-3">Quantity</label>
                <div className="flex items-center gap-4">
                  <div className="flex items-center border-2 border-amber-900/20 rounded overflow-hidden">
                    ...
                    <span className="px-6 py-2 border-x-2 border-amber-900/20">{quantity}</span>
                    <button
            ...
            <div className="bg-white rounded-lg p-8 border-2 border-amber-900/10 mt-6">
              <h3 className="font-semibold text-amber-900 text-xl mb-4">Product Details</h3>
              <dl className="space-y-3">
                <div className="flex justify-between">
                  <dt className="text-amber-700">Era:</dt>
                  <dd className="text-amber-900 font-medium">{product.details.era}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-amber-700">Condition:</dt>
                  <dd className="text-amber-900 font-medium">{product.details.condition}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-amber-700">Material:</dt>
                  <dd className="text-amber-900 font-medium">{product.details.material}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-amber-700">Care:</dt>
                  <dd className="text-amber-900 font-medium">{product.details.care}</dd>
                </div>
              </dl>
            </div>
          </div>
        ...
        <div className="mt-16 bg-white rounded-lg p-8 border-2 border-amber-900/10">
          <h3 className="font-semibold text-amber-900 text-2xl mb-6">Customer Reviews</h3>
